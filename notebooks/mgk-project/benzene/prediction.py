...
if args.n == 'best':
    ...
    dir_ = '%s/%d/%d' % (args.dir, args.seed, args.n)

if os.path.exists('%s/pred.csv' % dir_) and os.path.exists('%s/pred-auc' % dir_):
    exit(0)
...
open('%s/pred-auc' % dir_, 'w').write('%.5f' % roc_auc_score(test.y, y_pred))
